...
class OrderStats extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('Order', $this->getPageTableQuery()->count())
                ->chart($count->map(fn(TrendValue $item) => $item->aggregate)->toArray())
                ->icon('heroicon-o-shopping-bag')
                ->description('Order this month so far.')
            ...
            Stat::make('Total', 'Rp ' . number_format(
                Order::query()->where('status', OrderStatus::COMPLETED)
                    ->when($this->tableFilters['created_at']['created_from'] && $this->tableFilters['created_at']['created_until'],
                        fn($query) => $query->whereDate('created_at', '>=', $createdFrom)->whereDate('created_at', '<=', $createdTo)
                    )->sum('total'), 0, ',', '.'
            ))
                ->chart($count->map(fn(TrendValue $item) => $item->aggregate)->toArray())
                ->icon('heroicon-o-banknotes')
                ->description('Total this month so far.')
                ->descriptionColor('gray')
                ->color('success'),
